    ...
    if st.session_state.final_results is not None:
        ...
        with stats_spot:
            ...
            with col1:
                ...
                col1.metric("מינימום", f"{min_val:,.2f}₪  ({min_range})")
            with col2:
                max_val = final_stats['max_val']
                max_range = final_stats['max_date_range']
                col2.metric("מקסימום", f"{max_val:,.2f}₪  ({max_range})")

        with plot_spot:
            fig = px.histogram(x=final_arr, nbins=n_bins, title="",
                               labels={'x': '', 'y': 'שכיחות'}, color_discrete_sequence=['#1f77b4'])

            fig.update_xaxes(range=[min_val, max_val], automargin=True, showspikes=True, spikesnap="data")
            ...
            fig.update_traces(xbins=dict(start=min_val, end=max_val + bin_size, size=bin_size))
